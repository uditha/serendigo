...
// World/brand colors — same in light & dark
const FEATURE_SLIDES_CONFIG = [
  { emoji: '🗺️', title: 'Story Arcs', body: 'Follow curated journeys across the island — hidden temples, street food trails, wildlife encounters and more.', color: '#614A9E' },
  { emoji: '📸', title: 'Capture Moments', body: 'Visit locations in person, take a photo, and earn Serendipity Coins. No faking it — GPS verified.', color: '#1A5F8A' },
  { emoji: '📖', title: 'Your Passport', body: 'Collect province stamps as you explore. Build a living record of your Sri Lanka story.', color: '#E8832A' },
]
...
export default function WelcomeScreen() {
  ...
  const handleNext = () => {
    ...
    if (slideIndex < FEATURE_SLIDES_CONFIG.length - 1) {
      transition(step + 1)
    } else {
    ...
    <View style={[styles.container, { paddingTop: top, paddingBottom: bottom || spacing.lg }]}>
      <Animated.View style={[styles.content, { opacity: fadeAnim }]}>
        {step === 0 && (
          <View style={styles.stepContainer}>
            ...
            <View style={styles.bottomSection}>
              ...
              <Pressable style={[styles.primaryButton, { backgroundColor: colors.primary }]} onPress={() => transition(1)}>
        ...
        {step === 1 && (
          <View style={styles.stepContainer}>
            ...
            <View style={styles.choiceRow}>
              <Pressable style={styles.choiceCard} onPress={() => handleLocalChoice(false)}>
                ...
                <View style={[styles.choiceChip, { backgroundColor: '#1A5F8A20' }]}>
                  <Text style={[styles.choiceChipText, { color: '#1A5F8A' }]}>Traveller</Text>
                </View>
              </Pressable>
              <Pressable style={styles.choiceCard} onPress={() => handleLocalChoice(true)}>
                <Text style={styles.choiceEmoji}>🏝️</Text>
                ...
                <View style={[styles.choiceChip, { backgroundColor: '#614A9E20' }]}>
                  <Text style={[styles.choiceChipText, { color: '#614A9E' }]}>Islander</Text>
                </View>
              </Pressable>
            </View>
          </View>
        ...
        {step >= 2 && (
          <View style={styles.stepContainer}>
            {(() => {
              const slide = FEATURE_SLIDES_CONFIG[step - 2]
              const isLast = (step - 2) === FEATURE_SLIDES_CONFIG.length - 1
              return (
                <>
                  <View style={styles.topSection}>
                  ...
                  <View style={styles.bottomSection}>
                    ...
                    <Pressable style={[styles.primaryButton, { backgroundColor: slide.color }]} onPress={handleNext}>
