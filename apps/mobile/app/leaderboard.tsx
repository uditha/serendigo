...
const RANK_COLORS = ['#C9920A', '#BDC3C7', '#CD7F32']
const RANK_LABELS = ['🥇', '🥈', '🥉']
    ...
    >
        ...
        <View style={styles.myRankCard}>
          ...
          <View style={styles.myRankRow}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
              <CircleDollarSign size={18} color={colors.coinGold} />
      ...
      {!isLoading && data && data.length >= 3 && (
        <View style={styles.podium}>
          <PodiumCard entry={data[1]} highlight={data[1].id === user?.id} styles={styles} colors={colors} />
          <PodiumCard entry={data[0]} tall highlight={data[0].id === user?.id} styles={styles} colors={colors} />
          <PodiumCard entry={data[2]} highlight={data[2].id === user?.id} styles={styles} colors={colors} />
        </View>
      )}
        ...
        {isLoading ? (
          Array.from({ length: 8 }).map((_, i) => (
            <View key={i} style={styles.rowSkeleton}>
              <View style={{ width: 28, height: 28, borderRadius: 14, backgroundColor: colors.textTertiary + '30' }} />
              <View style={{ width: 28, height: 28, borderRadius: 14, backgroundColor: colors.textTertiary + '30' }} />
              <View style={{ flex: 1, gap: 4 }}>
                <View style={{ width: '50%', height: 14, borderRadius: 6, backgroundColor: colors.textTertiary + '30' }} />
                <View style={{ width: '30%', height: 11, borderRadius: 6, backgroundColor: colors.textTertiary + '30' }} />
              </View>
              <View style={{ width: 50, height: 14, borderRadius: 6, backgroundColor: colors.textTertiary + '30' }} />
            </View>
          ))
...
function PodiumCard({ entry, tall, highlight, styles, colors }: {
  entry: LeaderboardEntry; tall?: boolean; highlight?: boolean
  ...
  return (
    <View style={[styles.podiumCard, tall && styles.podiumCardTall, highlight && styles.podiumCardMe]}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3 }}>
        <CircleDollarSign size={12} color={colors.coinGold} />
...
function LeaderboardRow({ entry, isMe, styles, colors }: {
  ...
  return (
    <View style={[styles.row, isMe && styles.rowMe]}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
        <CircleDollarSign size={14} color={isMe ? colors.coinGold : colors.textSecondary} />
        <Text style={[styles.rowCoins, isMe && styles.rowCoinsMe]}>{entry.serendipityCoins}</Text>
      </View>
    </View>
  )
}
