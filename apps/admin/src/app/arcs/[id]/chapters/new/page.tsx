...
export default async function NewChapterPage({ params }: { params: { id: string } }) {
  const arc = (await db.select().from(arcs).where(eq(arcs.id, params.id)))[0]
  if (!arc) notFound()

  const [chapterCount] = await db.select({ count: count() }).from(chapters).where(eq(chapters.arcId, params.id))
